...
The following listing should aid you in understanding how the `async`/`await` algorithm ((("asynchronous functions", "await", id="af4ax")))iterates over a sequence of `await` expressions using a generator. Each item in the sequence is wrapped in a promise and then gets chained with the next step in the sequence. The promise returned by the underlying generator function becomes settled when the sequence ends or one of the promises is rejected.
