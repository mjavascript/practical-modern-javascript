...
Let's suppose ((("asynchronous functions", "flavors of async code", id="af4foac")))we had code like the following. Here I'm wrapping a `fetch` request in a `getRandomArticle` function. The promise fulfills with the JSON `body` when successful, and follows standard `fetch` rejection mechanics otherwise.
