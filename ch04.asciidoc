...
function getRandomArticle(gen) {
  const g = gen()
  //call next on the generator to force it to yield, and hold on that yield statement, 
  g.next();
  ...
  .then(res => res.json())
  //passing a value to next() here will cause the *prior* yield statement in the generator to resolve to the value we pass in, 
  //and will resume the generator, and run to the next yield. In this case there is none, so {done: true} will return
