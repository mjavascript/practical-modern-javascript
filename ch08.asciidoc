...
At the time of this writing, a proposal for dynamic `import()`​pass:[<span data-type="footnote">You can find the proposal <a href="https://mjavascript.com/out/dynamic-import">specification draft online</a>.</span>] expressions is sitting at stage 3 of the TC39 proposal review process. Unlike `import` statements, which are statically analyzed and linked, `import()` loads modules at runtime, returning a promise for the module namespace object after fetching, parsing, and executing the requested module and all of its dependencies.
