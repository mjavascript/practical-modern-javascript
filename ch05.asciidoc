...
The `Set#keys` method ((("Setkeysx")))also returns an iterator for values, again for consistency, and it's in fact a reference to the `Set#values` iterator.
...
Collections in ES6 provide built-in solutions for common use cases that were previously cumbersome to implement by users, such as the case of `Map`, or hard to execute correctly, as in the case of `WeakMap` where we allow references to be released if they're no longer interesting, avoiding memory ((("ES6 sets", startref="ess5")))((("Set", startref="set5")))((("ECMAScript", startref="ecmas5")))leaks.
