...
You could also create ((("arrow functions", "anonymous")))anonymous functions, by omitting the name, when assigning the function to a variable, a property, or a function call.
...
This ((("arrow functions", startref="af2")))is one of the most flexible and expressive features in ES6. It's also one of the simplest. It binds properties to as many variables as you need. It works with objects, arrays, and even in `function` parameter lists. Let's go step by step, starting with objects.
