...
This ((("arrow functions", startref="af2")))((("arrow functions", "merits and use cases", startref="af2mauc")))is one of the most flexible and expressive features in ES6. It's also one of the simplest. It binds properties to as many variables as you need. It works with objects, arrays, and even in `function` parameter lists. Let's go step by step, starting with objects.
