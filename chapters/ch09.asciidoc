...
Given that the views all have a similar API where they take a model and return an HTML string, we can treat them uniformly. If we wanted a +render+ function that could render any template, taking advantage of their shared API surface, we could easily do that by taking advantage of the dynamic nature of +require+, as shown next.
