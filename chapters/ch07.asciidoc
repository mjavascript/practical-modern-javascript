...
The `dotAll` proposalfootnote:[You can find the "'dotAll' flag proposal document at: https://mjavascript.com/out/regexp-dotall.] (stage 1) adds an `s` flag which changes the behavior of `.` in JavaScript regular expressions to match any single character.
