...
Having covered the essential aspects of ES6 in chapter 3, and symbols in chapter 4, we're now in great shape to understand promises, iterators, and generators. Promises offer a different way of attacking asynchronous code flows. Iterators dictate how an object is iterated, producing the sequence of values that gets iterated over. Generators can be used to write code that looks sequential but works asynchronously, in the background, as we'll learn towards the end of the chapter.
...
Binding several listeners for each type of event would eliminate the concern we had earlier about having to centralize concerns in a single callback. Events however make it hard to chain callbacks and have them fire when another asynchronous task is fulfilled, and that's where promises come in.
****
...
When a +p+ promise is fulfilled, reactions registered with +p.then+ are executed. When a +p+ promise is rejected, reactions registered with +p.catch+ are executed. Those reactions can, in turn, result in three different situations depending on whether they return a value, +throw+ an error, or return a +Promise+ or thenable. Thenables are objects considered promise-like that can be casted into a +Promise+ using +Promise.resolve+ as observed in section 5.1.3.
...
==== 5.1.3 Creating a Promise From Scratch
...
Note that you can transform data in rejection branches as well. Keep in mind that, as we first learned in section 5.1.3, when a +.catch+ reaction executes without errors, it will fulfill, following +.then+ reactions.
...
=== 5.2 Iterator Protocol and Iterable Protocol
...
image::../images/c05g01-halting-problem.png["The halting problem depicted by XKCD comic 1266"]
...
==== 5.2.3 Iterating Object Maps as Key-Value Pairs

There's an abundance of practical situations that benefit from turning an object into an iterable. Object maps, pseudo-arrays that are meant to be iterated, the random number generator we came up with in section 5.2.2, and classes or plain objects with properties that are often iterated could all turn a profit from following the iterable protocol.
...
Generators are an alternative way of creating functions that return an iterable object, without explicitly declaring an object literal with a +Symbol.iterator+ method. They make it easier to implement functions, such as the +range+ or +take+ functions in section 5.2.2, while also allowing for a few more interesting use cases.
...
The problem when taking this approach is that we're back at the case where the consumer didn't have control over how answers are used. To solve that, we could use the +g.next(value)+ message-passing feature that we discussed in section 5.3.3 as a way of forwarding answers to the user-provided generator.
...
As explained in section 5.2, You may recall how iterators leverage +Symbol.iterator+ as an interface to define how an object is to be iterated.
...
As highlighted in section 5.2.2, it's important to break out of this kinds of sequences, in order to avoid infinite loops.
